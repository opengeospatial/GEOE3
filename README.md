...
| 2. 	| [OUTPUT_CSV]   Destination CSV File 	|Full or relative file path and name. No extensions needed. | Output CSV (Comma Separated Values) file contains only errors aggregated per feature. If a file with the same name already exists in this folder, it will be overwritten. 	|
...
| 12. 	| [COORD_PREC]   Coordinate rounding precision (number of decimal places) 	| Integer 0-20 | The model rounds coordinates of the feature to a specified number of decimal places.	|
