...
| 3. 	| [OUTPUT_CITYGML] Destination CityGML file |File name with *extension*. | Output CityGML file contains invalid geometry parts and corresponding errors as an attributes. If the file with the same name already exists, it will be overwritten.<br /><br />*Note:* CityGML version needs an extension, CityJSON does not.| If empty, no output CityGML |
